...
__all__ = ['wcc_by_max_propagation']
...
def max_propagation(V, E):
    ...
    V_max = torch_scatter.scatter_max(V[E[0]], E[1], dim=0, dim_size=V.shape[0])[0]
...
def max_of_max_propagation(V, V_max):
    ...
    """
    V_max_max_cc = torch_scatter.scatter_max(V_max, V, dim=0)[0]
...
def connected_components_graph(E, V):
    ...
    min_idx_per_cc = torch_scatter.scatter_min(torch.arange(N, device=device), V)[0]

    # Assign new (dense) indices to the connected components, to be used
    # as nodes for the next (nested) propagation iteration
    I_cc = torch_geometric.nn.pool.consecutive.consecutive_cluster(min_idx_per_cc)[0]
...
def _wcc_by_max_propagation_without_isolated_nodes(
    ...
    I = torch_geometric.nn.pool.consecutive.consecutive_cluster(V_max)[0]
...
def wcc_by_max_propagation(
    ...
    :type E: torch.Tensor
    :param max_max: Whether the "max-of-max propagation trick" should be
        used. In practice, we recommend to keep this to False
    :type max_max: bool
    ...
    :return: Node-wise indices of the weakly-connected components and
        number of (recursive) iterations of the algorithm.
    ...
    if depth == 0:
        E, _ = torch_geometric.utils.remove_self_loops(E)
    ...
    if verbose:
        ...
        I_max = max_propagation(I_cc, E)
        num_diff = (I_cc != I_max).count_nonzero().item()
        num_cc = I_max.unique().shape[0]
        max_iterations = _max_iterations_max_prop(max_iterations, E)
        print(
            f"Time: {end - start:0.3f} | "
